# Adds a share button that ANY buyer can use while looking at a sale --
# not just the seller who posted it. Previously the only share button was
# on the seller's own "My Listings" screen; buyers browsing the map/list
# had no way to share a specific sale, so sharing "from the browser" just
# shared salehop.app's generic homepage link (no photo/title preview).
#
# Tap the new circle button (top-right of the listing sheet, next to the
# star) and:
#   - On a phone, it opens the phone's own native share sheet (Messages,
#     Instagram, WhatsApp, Facebook, email, etc. -- whatever's installed),
#     via the browser's built-in Web Share API.
#   - On a desktop browser (no native share sheet available), it falls
#     back to the same Facebook share popup already used elsewhere in the
#     app.
# Either way it shares the sale's own real page (salehop.app/listing/...),
# which has that sale's actual photo/title/address baked in as a proper
# link preview -- not the generic homepage card from your screenshot.
#
# New file: components/ShareButton.js
# Changed: components/ListingSheet.js (adds the button), app/globals.css
# (styles it to match the existing star button).
#
# No database changes, no new dependency. Safe to re-run if something
...
'@ | Set-Content -Encoding UTF8 "components\ShareButton.js"
...
/* Paid placement (see supabase/schema-v5-featured-listings.sql) -- a warm
   gold background wash across the whole card (a border-color tweak alone
   was too close to the default card's own yellow-ish left border to
   notice at a glance) plus the ".featured-badge" chip in .card-meta
   below. Comes before ".card.favorited" in the stylesheet on purpose: a
   sale can be both, and a personal "on my route" marker (green) is more
   useful to that one visitor than the paid-for badge, so favorited wins
   the left-border-color tiebreak when both apply -- the gold background
   still shows either way. */
.card.featured {
  background: linear-gradient(135deg, #FFF6D8 0%, #fff 60%);
...
.featured-badge { font-size: 11px; font-weight: 800; letter-spacing: 0.2px; background: var(--yellow); color: var(--ink); padding: 3px 8px; border-radius: 5px; box-shadow: 0 1px 2px rgba(224,160,32,0.45); }
...
.sheet-head .sheet-actions { margin-left: auto; display: flex; gap: 8px; flex-shrink: 0; }
.sheet-fav, .sheet-share {
...
.sheet-share { color: var(--ink-soft); }
.sheet-share:hover { background: var(--tan); }
...
.status-badge {
  ...
  letter-spacing: 0.4px; padding: 3px 8px; border-radius: 5px; margin: 0 6px 7px 0;
}
...
.status-badge.featured-pill { background: var(--yellow); color: var(--ink); box-shadow: 0 1px 2px rgba(224,160,32,0.45); }
.my-listing-card.featured { background: linear-gradient(135deg, #FFF6D8 0%, #fff 60%); border-color: var(--yellow-deep); }
...
.chip.featured-chip:disabled { background: #E5E0D3; border-color: var(--line); color: var(--ink-soft); opacity: 1; cursor: not-allowed; }
...
git add .
git commit -m "Add a share button to the listing sheet so any buyer can share a sale"
...
if ($LASTEXITCODE -eq 0) {
    Write-Host ""
    Write-Host "Done! Once Vercel finishes deploying, tap any sale to open its sheet -- you'll see a new round share button (a small arrow, top-right, next to the star). Try it on your phone for the native share sheet." -ForegroundColor Green
} else {
    Write-Host ""
