...
// "3h ago" / "2d ago" style relative timestamp -- used by the spotted-sale
// list cards and detail sheet (a sighting doesn't have a sale_date to show
// like a real listing does, just "when was this last reported/added").
...
// Combines a `YYYY-MM-DD` date and `HH:MM:SS` time (Postgres's own storage
// formats for sale_date/start_time/end_time) into a plain ISO 8601
// datetime string with no timezone offset -- e.g. "2026-08-17T09:00:00".
// Used only for the listing page's schema.org Event structured data;
// omitting an explicit offset is valid ISO 8601 and is read as the
// event's own local time, which is correct here since every sale is a
// real, single-timezone (Indiana) physical location.
