# Makes "Featured" listings actually stand out. The first version's
# highlight (a slightly different left-border color) was too close to
# every card's own default yellow-ish left border to notice while
# scrolling. This gives featured cards a warm gold background wash (both
# in Browse and in the seller's own My Listings), a bolder "Featured"
# badge, and a proper "Featured" pill next to the status badge in My
# Listings so a seller can immediately see which of their listings they
# paid for.
#
...
git add .
git commit -m "Make Featured listing highlight actually visible"
...
if ($LASTEXITCODE -eq 0) {
    Write-Host ""
    Write-Host "Done! Once Vercel finishes deploying, refresh Browse and your featured test listing should have an obvious gold background -- and in Account -> My Listings you'll see a yellow 'Featured' pill right next to the status badge." -ForegroundColor Green
} else {
    Write-Host ""
