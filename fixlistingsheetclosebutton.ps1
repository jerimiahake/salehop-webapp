# Fix: the listing sheet's "X" close button overlapped the favorite star
# once the title/address wrapped to more lines. Removed the X entirely --
# dragging the sheet all the way down already dismisses it back to the
# map (mouse drag on desktop works the same as a touch drag on a phone),
# so the button was redundant as well as buggy.
#
...
git add .
git commit -m "Fix: remove listing sheet close button (overlapped favorite star; drag-to-dismiss already covers it)"
...
if ($LASTEXITCODE -eq 0) {
    Write-Host ""
    Write-Host "Done! Once Vercel finishes deploying, the X should be gone -- just drag the sheet down to dismiss it." -ForegroundColor Green
} else {
    Write-Host ""
