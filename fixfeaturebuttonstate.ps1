# In My Listings, the "Feature -- $10" button used to disappear entirely
# once a listing was already featured -- easy to read as "did my payment
# even register?" Now it stays visible but turns gray and disabled,
# reading "Currently Featured" instead.
#
...
git add .
git commit -m "Gray out and relabel the Feature button once a listing is already featured"
...
if ($LASTEXITCODE -eq 0) {
    Write-Host ""
    Write-Host "Done! Once Vercel finishes deploying, a currently-featured listing in My Listings will show a grayed-out button that says Currently Featured instead of hiding it." -ForegroundColor Green
} else {
    Write-Host ""
